...
public:
    ...
    // Returns the serialized position
...
private:
    ...
    {
        if (index_ == 0)
            value_.push_back(0);    // Initialize next byte
...
static void serializeWhitesPosition(PositionSerializer * serializer, int const * board, int bar)
{
    for (int i = Board::SIZE - 1; i >= 0; --i)
    {
        ...
        serializer->pushOnes(whiteCount);
    }

    serializer->pushOnes(bar);
}
...
static void serializeBlacksPosition(PositionSerializer * serializer, int const * board, int bar)
{
    for (int i = 0; i < Board::SIZE; ++i)
    {
        ...
        serializer->pushOnes(blackCount);
    }
...
{
    // 1. For every point around the board (starting at the ace point of the player on roll, continuing around to the
    //    24 point and ending at the bar) :
    //      A. append as many 1s as the player on roll has on that point(if any).
    //      B. append a 0.
    // 2. For every point around the board (starting at the ace point of the opponent, continuing around to the
    //    opponent's 24 point and ending at the bar):
